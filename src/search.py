...
class NeuroLogSearch:
    def __init__(self, model_id="openai/clip-vit-base-patch32", index_name="neurolog"):
        # 1. Hardware & Precision Setup
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.dtype = torch.float16 if self.device == "cuda" else torch.float32
        
        print(f"[*] Initializing Search Node on {self.device.upper()} (Precision: {self.dtype})")
        ...
        if not os.path.exists(index_file) or not os.path.exists(meta_file):
            raise FileNotFoundError(f"[!] Database missing. Please run ingest.py first to generate the index.")
    ...
    def find_match(self, text_query, top_k=1):
        ...
        with torch.no_grad():
            ...
            # Normalize vector for FAISS Inner Product (Cosine Similarity)
        ...
        # Convert raw inner-product score to a percentage (0 to 100)
